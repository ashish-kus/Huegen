...
const int TOP_COLORS = 16;
const double COLOR_DIST_THRESHOLD = 30.0; // Increase if needed
...
int main(int argc, char **argv) {
  ...
  // Filter distinct top vibrant colors
  vector<Vec3b> finalColors;
  for (const auto &entry : vibrantColors) {
    const Vec3b &candidate = entry.second;
    bool isDistinct = true;
    for (const Vec3b &existing : finalColors) {
    ...
    if (isDistinct) {
      ...
      if (finalColors.size() == TOP_COLORS)
        break;
    }
  }
